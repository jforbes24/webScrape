...
try:
    for subCat in range(1):
        url = "https://www.diy.com/departments/heating-plumbing-cooling/DIY1652280.cat"
        result = requests.get(url, headers=headers)
        soup = bs4.BeautifulSoup(result.content, 'lxml')
        subCatMenu = soup.find('ul', class_='sc-1in1hqr-0 gkuWaM')
...
try:        
    for brick in subCatLinks:
        ...
        try:
            for brickCatMenu in broth.find_all('ul', class_='sc-1in1hqr-0 gkuWaM'):
...
try:
    ...
    # save to excel
    df.to_excel(r'C:\Users\forbej06\OneDrive - Kingfisher PLC\dev\Range\B&Q\HeatPlumb\bs4heatPlumb.xlsx', sheet_name='heatPlumb', index=False, header=True)
    data = pd.read_excel(r'C:\Users\forbej06\OneDrive - Kingfisher PLC\dev\Range\B&Q\HeatPlumb\bs4heatPlumb.xlsx')
    print(df)
except Exception as ex:
